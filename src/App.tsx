...
export default function App() {
  ...
  const runSimulation = () => {
    ...
    // A higher elasticity means demand changes more drastically with price
    ...
    const finalProducts = updatedProducts.map((p: any) => {
      if (
        p.id !== primaryNode.id &&
        p.categoryId === primaryNode.categoryId
      ) {
    ...
    // GIỮ NGUYÊN 100% ĐỂ ĐỒ THỊ KHÔNG BỊ GIÃN/LỖI
    graphData.nodes.forEach((n: any) => {
      if (n.type === "product") {
        const prod = finalProducts.find((p: any) => p.id === n.id);
        if (prod) {
          n.price = prod.price;
          n.demand = prod.demand;
        }
      }
    });

    updatedProducts.forEach((p: any) => {
      const match = finalProducts.find((fp: any) => fp.id === p.id);
      if (!match) finalProducts.push(p);
    });

    // === SỬA LỖI COMPARE MODE Ở ĐÂY ===
    // Dùng map và clone object ({...s}) để tạo ra lịch sử dữ liệu tách biệt
    const updatedSellers = graphData.sellers ? graphData.sellers.map((s: any) => {
    ...
    const updatedCategories = graphData.categories ? graphData.categories.map((c: any) => {
    ...
    // Cập nhật lại với mảng sellers và categories mới
    ...
    setSelectedNodes(prev => prev.map(n => {
      ...
      if (n.type === 'category') return updatedGraphData.categories?.find((c: any) => c.id === n.id) || n;
      if (n.type === 'seller') return updatedGraphData.sellers?.find((s: any) => s.id === n.id) || n;
      return n;
    }));
    
    // GIỮ NGUYÊN BẢN (Bạn đã làm phần này rất đúng rồi)
    if (scenarioNameInput) {
       saveScenarioByName(scenarioNameInput);
    } else {
       saveScenarioByName("Điều chỉnh giá: " + primaryNode.name);
    }
  };
